...
let paragraphs = document.querySelector("#amount");

genBtn.addEventListener('click', (data) => {
    data.preventDefault();
    let textPrint = [];
    if(paragraphs.value < 10 & paragraphs.value > 0) {
        for (let i=0; i<paragraphs.value; i++){
            let parag = Math.floor(Math.random() * Math.floor(9));
            textPrint.push(text[parag])
        
        }
        let createText = textPrint.join('<br><br>')
        let textCard = document.querySelector(".lorem-text");
        textCard.innerHTML = createText;
    }
    else{
        alert("Please, choose between 1 & 9 paragraphs");
